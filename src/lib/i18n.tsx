import { createContext, useContext, useEffect, useState, ReactNode } from "react";
import { setUserLangPreference } from "./lang-detect";
import { pl } from "./i18n-pl-dict";

export type Lang = "en" | "zh" | "es" | "pt" | "pl";

export type Dict = Record<string, string>;

export const localeMap: Record<Lang, string> = {
  en: "en-US",
  zh: "zh-CN",
  es: "es-ES",
  pt: "pt-BR",
  pl: "pl-PL",
};

export function formatTime(d: Date, lang: Lang) {
  // Brasil e Polônia usam formato 24h (convenções pt-BR e pl-PL).
  // Demais idiomas mantêm 12h.
  const hour12 = lang !== "pt" && lang !== "pl";
  return d.toLocaleTimeString(localeMap[lang], {
    hour: "2-digit",
    minute: "2-digit",
    hour12,
  });
}

export function useFmtTime() {
  const { lang } = useI18n();
  return (d: Date) => formatTime(d, lang);
}

const en: Dict = {
  "nav.home": "Home",
  "nav.calculator": "Calculator",
  "nav.program": "Program",
  "nav.assessment": "Assessment",
  "nav.diary": "Diary",
  "nav.relax": "Relax",
  "nav.learn": "Learn",
  "nav.dashboard": "Dashboard",
  "nav.tools": "Tools",
  "nav.tools.section": "Sleep Calculators",
  "nav.guides": "CBT-I Guides",
  "nav.lessons": "Sleep Lessons",
  "cta.start": "Start Sleep Plan",
  "cta.calc": "Try Sleep Calculator",
  "hero.title": "Sleep Better, Starting Tonight",
  "hero.sub":
    "A science-based CBT-I sleep platform designed to help you restore healthy sleep patterns naturally.",
  "features.title": "A gentle path back to restful nights",
  "features.1.t": "CBT-I Program",
  "features.1.d": "Step-by-step guidance to improve sleep habits and rebuild healthy sleep rhythm.",
  "features.2.t": "Sleep Calculator",
  "features.2.d": "Find your ideal bedtime and wake time using natural sleep cycles.",
  "features.3.t": "Relax & Wind Down",
  "features.3.d": "Breathing, meditation, and calming nighttime rituals.",
  "trust.title": "Backed by science. Built for calm.",
  "trust.sub":
    "CBT-I is the gold-standard, drug-free approach recommended by sleep specialists worldwide. Small, gentle changes — over weeks — rebuild your natural rhythm.",
  "trust.1.t": "Evidence-based",
  "trust.1.d":
    "Decades of clinical research support CBT-I as the first-line treatment for chronic insomnia.",
  "trust.2.t": "Drug-free",
  "trust.2.d": "No medication. No side effects. Just structured habits that retrain your brain.",
  "trust.3.t": "Habit-based recovery",
  "trust.3.d": "Tiny, sustainable shifts in your routine restore healthy sleep over time.",
  "emo.title": "If your nights feel like this…",
  "emo.1": "Lying awake long after the lights go out",
  "emo.2": "A racing mind that won't quiet down",
  "emo.3": "Exhausted, but somehow unable to sleep",
  "emo.4": "Anxiety creeping in as bedtime approaches",
  "emo.note": "You're not alone. Sleep can be relearned — gently, gradually, and without pressure.",
  "calc.title": "Sleep Cycle Calculator",
  "calc.sub":
    "Sleep happens in ~90 minute cycles. Waking at the end of a cycle helps you feel refreshed.",
  "calc.wake": "I want to wake up at",
  "calc.bed": "I want to fall asleep at",
  "calc.mode.wake": "Plan around wake time",
  "calc.mode.bed": "Plan around bedtime",
  "calc.results.bed": "Try going to bed at one of these times",
  "calc.results.wake": "Try waking at one of these times",
  "calc.cycles": "cycles",
  "calc.fall": "Assumes ~15 min to fall asleep.",
  "calc.tier.4.label": "Minimum sleep",
  "calc.tier.5.label": "Best for most adults",
  "calc.tier.6.label": "Extra recovery",
  "calc.tier.7.label": "Useful after sleep debt",
  "calc.badge.recommended": "⭐ Recommended",
  "calc.badge.recovery": "Recovery",
  "calc.explain.title": "Why are 5 and 6 cycles recommended?",
  "calc.explain.body":
    "Most adults function best with 7.5–9 hours of sleep. Sleeping in complete 90-minute cycles may help reduce morning grogginess and improve daytime energy.",
  "program.title": "The CBT-I Program",
  "program.sub": "A 6-week gentle journey to rebuild your sleep, one small step at a time.",
  "assess.title": "Sleep Assessment",
  "assess.sub": "A short, supportive check-in to understand your sleep.",
  "assess.start": "Begin assessment",
  "assess.next": "Next",
  "assess.back": "Back",
  "assess.done": "See my sleep profile",
  "assess.q1": "How long does it usually take you to fall asleep?",
  "assess.q2": "How often do you wake up during the night?",
  "assess.q3": "Roughly how many hours do you sleep?",
  "assess.q4": "How would you describe your bedtime anxiety?",
  "assess.q5": "How rested do you feel when you wake up?",
  "assess.support":
    "Many people experience this. Sleep can improve gradually with structure and consistency.",
  "assess.result": "Your sleep profile",
  "assess.result.sub": "A starting point, not a diagnosis. Your CBT-I plan is built around this.",
  "assess.cta.program": "Start program",
  "assess.cta.dashboard": "Go to dashboard",
  "assess.level.label": "Sleep profile",
  "assess.level1.name": "Healthy sleeper",
  "assess.level1.desc": "Your sleep habits look healthy.",
  "assess.level2.name": "Mild sleep difficulties",
  "assess.level2.desc": "There are some mild sleep-related challenges.",
  "assess.level3.name": "Moderate insomnia pattern",
  "assess.level3.desc": "CBT-I techniques may help improve your sleep.",
  "assess.level4.name": "Significant insomnia pattern",
  "assess.level4.desc": "A structured CBT-I program is recommended.",
  "assess.health.label": "Sleep health",
  "assess.health.note": "Higher means healthier sleep habits.",
  "assess.stat.efficiency": "Sleep efficiency",
  "assess.stat.efficiency.value": "On track",
  "assess.stat.program": "Recommended program",
  "assess.stat.program.value": "CBT-I journey",
  "assess.stat.duration": "Program duration",
  "assess.stat.duration.value": "6 weeks",
  "assess.stat.efficiency.value.l1": "Excellent",
  "assess.stat.efficiency.value.l2": "Good",
  "assess.stat.efficiency.value.l3": "Fair",
  "assess.stat.efficiency.value.l4": "Needs attention",
  "assess.stat.program.value.l1": "Relaxation routine",
  "assess.stat.program.value.l2": "Light CBT-I habits",
  "assess.stat.program.value.l3": "Core CBT-I program",
  "assess.stat.program.value.l4": "Full CBT-I program",
  "assess.stat.duration.value.l1": "1–2 weeks",
  "assess.stat.duration.value.l2": "2–4 weeks",
  "assess.stat.duration.value.l3": "4–6 weeks",
  "assess.stat.duration.value.l4": "6 weeks",
  "diary.title": "Sleep Diary",
  "diary.sub": "A gentle, judgment-free log helps reveal what's already working.",
  "diary.bedtime": "Bedtime",
  "diary.fall": "Time to fall asleep (min)",
  "diary.wakes": "Nighttime awakenings",
  "diary.wake": "Wake time",
  "diary.quality": "Sleep quality",
  "diary.mood": "Morning mood",
  "diary.save": "Save entry",
  "diary.cta.dashboard": "View dashboard",
  "diary.saved": "Entry saved 🌙",
  "dash.title": "Tonight's sleep plan",
  "dash.bed": "Suggested bedtime",
  "dash.wake": "Wake time",
  "dash.eff": "Sleep efficiency",
  "dash.streak": "Consistency streak",
  "dash.actions": "Quick actions",
  "relax.title": "Relax & Wind Down",
  "relax.sub": "A quiet space to let go of the day.",
  "relax.breathe": "4-7-8 Breathing",
  "relax.breathe.sub": "Inhale 4 · Hold 7 · Exhale 8",
  "relax.inhale": "Inhale",
  "relax.hold": "Hold",
  "relax.exhale": "Exhale",
  "relax.start": "Start session",
  "relax.stop": "Stop",
  "learn.title": "Learn",
  "learn.sub": "Short reads to understand sleep and yourself.",
  "footer.tag": "A gentle sleep companion guided by science.",
  "footer.rights": "© 2026 somna. All rights reserved.",
  "footer.disc": "Not medical advice. For persistent sleep issues, consult a clinician.",
  "home.return.title": "Continue your sleep journey",
  "home.return.cta": "Go to dashboard",
  "share.title": "Share Your Progress",
  "share.download": "Download Image",
  "share.share": "Share",
  "share.cancel": "Cancel",
  "share.reddit": "Reddit",
  "share.x": "X",
  "share.pinterest": "Pinterest",
  "share.facebook": "Facebook",
  "share.linkedin": "LinkedIn",
  "share.whatsapp": "WhatsApp",
  "share.pinterestFallback": "Please download the image first, then share it to Pinterest.",
  "share.copyLink": "Copy Link",
  "share.copied": "Link copied",
  "share.copyFailed": "Could not copy link",
  "share.failed": "Share failed. Please try again.",
  "share.uploadError": "Image upload failed. Please check your connection and try again.",
  "share.previewTitle": "Preview",
  "share.hidePreview": "Hide preview",
  "share.showPreview": "Show preview",
  "share.errorTip": "You can still download the image and share it manually.",
  "share.pngFormatInvalid": "Only PNG images are supported.",
  "share.fileSizeOverLimit": "Image must be smaller than 5 MB.",
  "share.shareLesson": "Share This Lesson",
  "share.shareProfile": "Share Your Sleep Profile",
  "share.downloadProfile": "Download Profile",
  "share.shareProfileBtn": "Share Profile",
  "share.downloadResult": "Download Result",
  "share.shareResult": "Share Result",
  "share.sleepEfficiency": "Sleep Efficiency",
  "share.improving": "Improving",
  "share.dayStreak": "Day Streak",
  "share.cbtiTraining": "CBT-I Sleep Training",
  "share.sleepProfile": "Sleep Profile",
  "share.sleepType": "Sleep Type",
  "share.wechat": "WeChat",
  "share.weibo": "Weibo",
  "share.qq": "QQ",
  "share.wechatTitle": "Share to WeChat",
  "share.wechatScanHint": "Scan this QR Code with WeChat",
  "share.wechatLongPress": "Long press the image to share.",
  "share.wechatPreviewImage": "Preview Image",
  "share.downloadImage": "Download Image",
  "share.downloadStarted": "Download started.",
  "share.lang": "EN",
  lang: "EN",
};

const zh: Dict = {
  "nav.home": "首页",
  "nav.calculator": "睡眠计算",
  "nav.program": "课程",
  "nav.assessment": "测评",
  "nav.diary": "睡眠日记",
  "nav.relax": "放松",
  "nav.learn": "知识",
  "nav.dashboard": "仪表盘",
  "cta.start": "开启睡眠计划",
  "cta.calc": "试试睡眠计算器",
  "hero.title": "今晚开始,睡得更好",
  "hero.sub": "基于 CBT-I 的科学方法,温柔地帮助你重建健康的睡眠节律。",
  "features.title": "回到安稳夜晚的温柔之路",
  "features.1.t": "CBT-I 课程",
  "features.1.d": "循序渐进的引导,改善睡眠习惯,重建健康节律。",
  "features.2.t": "睡眠计算器",
  "features.2.d": "依据自然睡眠周期,找到最适合你的入睡与起床时间。",
  "features.3.t": "放松与入眠",
  "features.3.d": "呼吸、冥想与平静的睡前仪式。",
  "trust.title": "由科学支持,为平静而生。",
  "trust.sub":
    "CBT-I 是被全球睡眠专家推荐的、无药物的一线方法。每一个微小、温柔的改变,都会逐步重建你的自然节律。",
  "trust.1.t": "循证有效",
  "trust.1.d": "数十年临床研究支持 CBT-I 作为慢性失眠的首选疗法。",
  "trust.2.t": "无需药物",
  "trust.2.d": "没有药物,没有副作用,只有结构化的习惯,温柔重塑大脑。",
  "trust.3.t": "习惯式恢复",
  "trust.3.d": "微小、可持续的日常调整,慢慢恢复健康睡眠。",
  "emo.title": "如果你的夜晚是这样的……",
  "emo.1": "熄灯后久久无法入睡",
  "emo.2": "思绪不停,无法平静",
  "emo.3": "明明很累,却睡不着",
  "emo.4": "临近睡前,焦虑悄悄涌来",
  "emo.note": "你并不孤单。睡眠是可以被重新学会的——温柔、缓慢、没有压力。",
  "calc.title": "睡眠周期计算器",
  "calc.sub": "睡眠以约 90 分钟为一个周期。在周期末尾醒来,会更清爽。",
  "calc.wake": "我想在以下时间醒来",
  "calc.bed": "我想在以下时间入睡",
  "calc.mode.wake": "按起床时间规划",
  "calc.mode.bed": "按入睡时间规划",
  "calc.results.bed": "试着在这些时间上床",
  "calc.results.wake": "试着在这些时间醒来",
  "calc.cycles": "个周期",
  "calc.fall": "已加入约 15 分钟入睡时间。",
  "calc.tier.4.label": "最低建议",
  "calc.tier.5.label": "适合大多数成年人",
  "calc.tier.6.label": "额外恢复睡眠",
  "calc.tier.7.label": "适用于补觉或恢复",
  "calc.badge.recommended": "⭐ 推荐",
  "calc.badge.recovery": "恢复",
  "calc.explain.title": "为什么推荐 5 或 6 个睡眠周期？",
  "calc.explain.body":
    "大多数成年人在 7.5–9 小时睡眠时状态最佳。按照完整的 90 分钟周期安排睡眠，有助于减少起床后的疲惫感，提高白天精力。",
  "program.title": "CBT-I 课程",
  "program.sub": "为期 6 周的温柔旅程,一步一步重建你的睡眠。",
  "assess.title": "睡眠测评",
  "assess.sub": "一份简短、温柔的自我了解。",
  "assess.start": "开始测评",
  "assess.next": "下一题",
  "assess.back": "上一题",
  "assess.done": "查看我的睡眠画像",
  "assess.q1": "你通常需要多久才能入睡?",
  "assess.q2": "夜间醒来的频率?",
  "assess.q3": "你大约睡多少小时?",
  "assess.q4": "睡前焦虑的程度?",
  "assess.q5": "醒来时感到休息得如何?",
  "assess.support": "很多人都有类似的情况。通过结构与一致性,睡眠可以慢慢改善。",
  "assess.result": "你的睡眠画像",
  "assess.result.sub": "这是一个起点,而非诊断。你的 CBT-I 计划将围绕它展开。",
  "assess.cta.program": "开始我的计划",
  "assess.cta.dashboard": "进入仪表盘",
  "assess.level.label": "睡眠画像",
  "assess.level1.name": "健康睡眠型",
  "assess.level1.desc": "你的睡眠习惯总体良好。",
  "assess.level2.name": "轻度睡眠困难型",
  "assess.level2.desc": "存在一些轻微的睡眠问题。",
  "assess.level3.name": "中度失眠模式",
  "assess.level3.desc": "CBT-I 方法可能帮助改善睡眠。",
  "assess.level4.name": "明显失眠模式",
  "assess.level4.desc": "建议进行系统性的 CBT-I 训练。",
  "assess.health.label": "睡眠健康度",
  "assess.health.note": "越高代表睡眠习惯越健康。",
  "assess.stat.efficiency": "睡眠效率",
  "assess.stat.efficiency.value": "保持中",
  "assess.stat.program": "推荐计划",
  "assess.stat.program.value": "CBT-I 之旅",
  "assess.stat.duration": "计划周期",
  "assess.stat.duration.value": "6 周",
  "assess.stat.efficiency.value.l1": "优秀",
  "assess.stat.efficiency.value.l2": "良好",
  "assess.stat.efficiency.value.l3": "一般",
  "assess.stat.efficiency.value.l4": "需关注",
  "assess.stat.program.value.l1": "放松入眠练习",
  "assess.stat.program.value.l2": "轻度 CBT-I 习惯",
  "assess.stat.program.value.l3": "核心 CBT-I 课程",
  "assess.stat.program.value.l4": "完整 CBT-I 课程",
  "assess.stat.duration.value.l1": "1–2 周",
  "assess.stat.duration.value.l2": "2–4 周",
  "assess.stat.duration.value.l3": "4–6 周",
  "assess.stat.duration.value.l4": "6 周",
  "diary.title": "睡眠日记",
  "diary.sub": "温柔地记录,不评判,帮助你看见已经在起作用的部分。",
  "diary.bedtime": "上床时间",
  "diary.fall": "入睡所需时间(分钟)",
  "diary.wakes": "夜间醒来次数",
  "diary.wake": "起床时间",
  "diary.quality": "睡眠质量",
  "diary.mood": "晨间心情",
  "diary.save": "保存记录",
  "diary.cta.dashboard": "查看仪表盘",
  "diary.saved": "已保存 🌙",
  "dash.title": "今晚的睡眠计划",
  "dash.bed": "建议上床时间",
  "dash.wake": "起床时间",
  "dash.eff": "睡眠效率",
  "dash.streak": "坚持天数",
  "dash.actions": "快捷操作",
  "relax.title": "放松与入眠",
  "relax.sub": "一个让一天慢慢落下的安静角落。",
  "relax.breathe": "4-7-8 呼吸法",
  "relax.breale": "吸气",
  "relax.hold": "屏息",
  "relax.exhale": "呼气",
  "relax.start": "开始",
  "relax.stop": "停止",
  "learn.title": "知识",
  "learn.sub": "理解睡眠,也理解自己。",
  "footer.tag": "由科学引导的温柔睡眠伙伴。",
  "footer.rights": "© 2026 somna. 保留所有权利。",
  "footer.disc": "本内容不构成医疗建议。如有持续的睡眠问题,请咨询专业医生。",
  "home.return.title": "继续你的睡眠计划",
  "home.return.cta": "进入仪表盘",
  "share.title": "分享你的进展",
  "share.download": "下载图片",
  "share.share": "分享",
  "share.cancel": "取消",
  "share.reddit": "Reddit",
  "share.x": "X",
  "share.pinterest": "Pinterest",
  "share.facebook": "Facebook",
  "share.linkedin": "LinkedIn",
  "share.whatsapp": "WhatsApp",
  "share.pinterestFallback": "请先生成分享图片，再分享到 Pinterest。",
  "share.copyLink": "复制链接",
  "share.copied": "链接已复制",
  "share.copyFailed": "无法复制链接",
  "share.failed": "分享失败，请重试",
  "share.uploadError": "图片上传失败，请检查网络后重试",
  "share.previewTitle": "预览",
  "share.hidePreview": "隐藏预览",
  "share.showPreview": "显示预览",
  "share.errorTip": "你仍然可以下载图片并手动分享",
  "share.pngFormatInvalid": "仅支持 PNG 图片",
  "share.fileSizeOverLimit": "图片必须小于 5 MB",
  "share.shareLesson": "分享这一课",
  "share.shareProfile": "分享你的睡眠档案",
  "share.downloadProfile": "下载档案",
  "share.shareProfileBtn": "分享档案",
  "share.downloadResult": "下载结果",
  "share.shareResult": "分享结果",
  "share.sleepEfficiency": "睡眠效率",
  "share.improving": "正在改善",
  "share.dayStreak": "天连续记录",
  "share.cbtiTraining": "CBT-I 睡眠训练",
  "share.sleepProfile": "睡眠档案",
  "share.sleepType": "睡眠类型",
  "share.wechat": "微信",
  "share.weibo": "微博",
  "share.qq": "QQ",
  "share.wechatTitle": "分享到微信",
  "share.wechatScanHint": "用微信扫描此二维码",
  "share.wechatLongPress": "长按图片即可分享",
  "share.wechatPreviewImage": "预览图片",
  "share.downloadImage": "下载图片",
  "share.downloadStarted": "已开始下载",
  "share.lang": "中",
  lang: "中",
};

const es: Dict = {
  "nav.home": "Inicio",
  "nav.calculator": "Calculadora",
  "nav.program": "Programa",
  "nav.assessment": "Evaluación",
  "nav.diary": "Diario",
  "nav.relax": "Relajación",
  "nav.learn": "Aprender",
  "nav.dashboard": "Panel",
  "nav.tools": "Herramientas",
  "nav.tools.section": "Calculadoras del sueño",
  "nav.guides": "Guías CBT-I",
  "nav.lessons": "Lecciones",
  "cta.start": "Comenzar plan de sueño",
  "cta.calc": "Probar calculadora de sueño",
  "hero.title": "Duerme mejor, desde esta noche",
  "hero.sub":
    "Una plataforma de sueño basada en CBT-I, diseñada para ayudarte a recuperar patrones de sueño saludables de forma natural.",
  "features.title": "Un camino suave de regreso a noches reparadoras",
  "features.1.t": "Programa CBT-I",
  "features.1.d":
    "Guía paso a paso para mejorar hábitos y reconstruir un ritmo de sueño saludable.",
  "features.2.t": "Calculadora de sueño",
  "features.2.d":
    "Encuentra tu hora ideal para dormir y despertar usando ciclos naturales de sueño.",
  "features.3.t": "Relajación y descanso",
  "features.3.d": "Respiración, meditación y rituales calmantes antes de dormir.",
  "trust.title": "Respaldado por la ciencia. Hecho para la calma.",
  "trust.sub":
    "CBT-I es el método estándar sin medicamentos recomendado por especialistas del sueño en todo el mundo. Pequeños cambios suaves, durante semanas, reconstruyen tu ritmo natural.",
  "trust.1.t": "Basado en evidencia",
  "trust.1.d":
    "Décadas de investigación clínica respaldan a CBT-I como tratamiento de primera línea para el insomnio crónico.",
  "trust.2.t": "Sin medicamentos",
  "trust.2.d":
    "Sin fármacos. Sin efectos secundarios. Solo hábitos estructurados que reentrenan tu cerebro.",
  "trust.3.t": "Recuperación basada en hábitos",
  "trust.3.d":
    "Pequeños ajustes sostenibles en tu rutina restauran un sueño saludable con el tiempo.",
  "emo.title": "Si tus noches se sienten así…",
  "emo.1": "Despierto mucho tiempo después de apagar las luces",
  "emo.2": "Una mente acelerada que no se calma",
  "emo.3": "Agotado, pero sin poder dormir",
  "emo.4": "La ansiedad aparece a medida que se acerca la hora de dormir",
  "emo.note": "No estás solo. El sueño puede reaprenderse, suavemente, gradualmente y sin presión.",
  "calc.title": "Calculadora de ciclos de sueño",
  "calc.sub":
    "El sueño ocurre en ciclos de ~90 minutos. Despertar al final de un ciclo te ayuda a sentirte renovado.",
  "calc.wake": "Quiero despertar a las",
  "calc.bed": "Quiero quedarme dormido a las",
  "calc.mode.wake": "Planificar por hora de despertar",
  "calc.mode.bed": "Planificar por hora de dormir",
  "calc.results.bed": "Intenta acostarte a una de estas horas",
  "calc.results.wake": "Intenta despertar a una de estas horas",
  "calc.cycles": "ciclos",
  "calc.fall": "Asume ~15 min para conciliar el sueño.",
  "calc.tier.4.label": "Sueño mínimo",
  "calc.tier.5.label": "Ideal para la mayoría de los adultos",
  "calc.tier.6.label": "Recuperación adicional",
  "calc.tier.7.label": "Útil para recuperar el sueño",
  "calc.badge.recommended": "⭐ Recomendado",
  "calc.badge.recovery": "Recuperación",
  "calc.explain.title": "¿Por qué se recomiendan 5 o 6 ciclos?",
  "calc.explain.body":
    "La mayoría de los adultos se sienten mejor con 7.5–9 horas de sueño. Dormir en ciclos completos de 90 minutos puede ayudar a reducir la somnolencia al despertar y mejorar la energía durante el día.",
  "program.title": "El programa CBT-I",
  "program.sub": "Un viaje suave de 6 semanas para reconstruir tu sueño, paso a paso.",
  "assess.title": "Evaluación del sueño",
  "assess.sub": "Una breve y amable revisión para entender tu sueño.",
  "assess.start": "Comenzar evaluación",
  "assess.next": "Siguiente",
  "assess.back": "Atrás",
  "assess.done": "Ver mi perfil de sueño",
  "assess.q1": "¿Cuánto tardas usualmente en quedarte dormido?",
  "assess.q2": "¿Con qué frecuencia te despiertas durante la noche?",
  "assess.q3": "¿Cuántas horas duermes aproximadamente?",
  "assess.q4": "¿Cómo describirías tu ansiedad al acostarte?",
  "assess.q5": "¿Qué tan descansado te sientes al despertar?",
  "assess.support":
    "Mucha gente vive esto. El sueño puede mejorar gradualmente con estructura y constancia.",
  "assess.result": "Tu perfil de sueño",
  "assess.result.sub":
    "Un punto de partida, no un diagnóstico. Tu plan CBT-I se construye en torno a esto.",
  "assess.cta.program": "Comenzar programa",
  "assess.cta.dashboard": "Ir al panel",
  "assess.level.label": "Perfil de sueño",
  "assess.level1.name": "Dormidor saludable",
  "assess.level1.desc": "Tus hábitos de sueño parecen saludables.",
  "assess.level2.name": "Dificultades leves para dormir",
  "assess.level2.desc": "Existen algunos desafíos leves relacionados con el sueño.",
  "assess.level3.name": "Patrón moderado de insomnio",
  "assess.level3.desc": "Las técnicas de CBT-I pueden ayudarte a mejorar el sueño.",
  "assess.level4.name": "Patrón significativo de insomnio",
  "assess.level4.desc": "Se recomienda seguir un programa estructurado de CBT-I.",
  "assess.health.label": "Salud del sueño",
  "assess.health.note": "Más alto significa hábitos de sueño más saludables.",
  "assess.stat.efficiency": "Eficiencia del sueño",
  "assess.stat.efficiency.value": "En camino",
  "assess.stat.program": "Programa recomendado",
  "assess.stat.program.value": "Viaje CBT-I",
  "assess.stat.duration": "Duración del programa",
  "assess.stat.duration.value": "6 semanas",
  "assess.stat.efficiency.value.l1": "Excelente",
  "assess.stat.efficiency.value.l2": "Bueno",
  "assess.stat.efficiency.value.l3": "Regular",
  "assess.stat.efficiency.value.l4": "Necesita atención",
  "assess.stat.program.value.l1": "Rutina de relajación",
  "assess.stat.program.value.l2": "Hábitos ligeros de CBT-I",
  "assess.stat.program.value.l3": "Programa central de CBT-I",
  "assess.stat.program.value.l4": "Programa completo de CBT-I",
  "assess.stat.duration.value.l1": "1–2 semanas",
  "assess.stat.duration.value.l2": "2–4 semanas",
  "assess.stat.duration.value.l3": "4–6 semanas",
  "assess.stat.duration.value.l4": "6 semanas",
  "diary.title": "Diario de sueño",
  "diary.sub": "Un registro suave, sin juicio, ayuda a revelar lo que ya está funcionando.",
  "diary.bedtime": "Hora de acostarse",
  "diary.fall": "Tiempo para dormirse (min)",
  "diary.wakes": "Despertares nocturnos",
  "diary.wake": "Hora de despertar",
  "diary.quality": "Calidad del sueño",
  "diary.mood": "Ánimo matutino",
  "diary.save": "Guardar entrada",
  "diary.cta.dashboard": "Ver panel",
  "diary.saved": "Entrada guardada 🌙",
  "dash.title": "Plan de sueño de esta noche",
  "dash.bed": "Hora sugerida para dormir",
  "dash.wake": "Hora de despertar",
  "dash.eff": "Eficiencia del sueño",
  "dash.streak": "Racha de constancia",
  "dash.actions": "Acciones rápidas",
  "relax.title": "Relajación y descanso",
  "relax.sub": "Un espacio tranquilo para dejar ir el día.",
  "relax.breathe": "Respiración 4-7-8",
  "relax.breathe.sub": "Inhala 4 · Sostén 7 · Exhala 8",
  "relax.inhale": "Inhala",
  "relax.hold": "Sostén",
  "relax.exhale": "Exhala",
  "relax.start": "Comenzar sesión",
  "relax.stop": "Detener",
  "learn.title": "Aprender",
  "learn.sub": "Lecturas breves para entender el sueño y a ti mismo.",
  "footer.tag": "Un compañero de sueño suave guiado por la ciencia.",
  "footer.rights": "© 2026 somna. Todos los derechos reservados.",
  "footer.disc":
    "No es consejo médico. Para problemas de sueño persistentes, consulta a un clínico.",
  "home.return.title": "Continúa tu viaje del sueño",
  "home.return.cta": "Ir al panel",
  "share.title": "Comparte tu progreso",
  "share.download": "Descargar imagen",
  "share.share": "Compartir",
  "share.cancel": "Cancelar",
  "share.reddit": "Reddit",
  "share.x": "X",
  "share.pinterest": "Pinterest",
  "share.facebook": "Facebook",
  "share.linkedin": "LinkedIn",
  "share.whatsapp": "WhatsApp",
  "share.pinterestFallback":
    "Primero genera la imagen para compartir y luego publícala en Pinterest.",
  "share.copyLink": "Copiar enlace",
  "share.copied": "Enlace copiado",
  "share.copyFailed": "No se pudo copiar el enlace",
  "share.failed": "Error al compartir. Inténtalo de nuevo.",
  "share.uploadError": "Error al subir la imagen. Comprueba tu conexión.",
  "share.previewTitle": "Vista previa",
  "share.hidePreview": "Ocultar vista previa",
  "share.showPreview": "Mostrar vista previa",
  "share.errorTip": "Aún puedes descargar la imagen y compartirla manualmente.",
  "share.pngFormatInvalid": "Solo se admiten imágenes PNG.",
  "share.fileSizeOverLimit": "La imagen debe ser menor a 5 MB.",
  "share.shareLesson": "Comparte esta lección",
  "share.shareProfile": "Comparte tu perfil de sueño",
  "share.downloadProfile": "Descargar perfil",
  "share.shareProfileBtn": "Compartir perfil",
  "share.downloadResult": "Descargar resultado",
  "share.shareResult": "Compartir resultado",
  "share.sleepEfficiency": "Eficiencia del sueño",
  "share.improving": "Mejorando",
  "share.dayStreak": "Días seguidos",
  "share.cbtiTraining": "Entrenamiento de sueño CBT-I",
  "share.sleepProfile": "Perfil de sueño",
  "share.sleepType": "Tipo de sueño",
  "share.wechat": "WeChat",
  "share.weibo": "Weibo",
  "share.qq": "QQ",
  "share.wechatTitle": "Compartir en WeChat",
  "share.wechatScanHint": "Escanea este código QR con WeChat",
  "share.wechatLongPress": "Mantén pulsada la imagen para compartirla.",
  "share.wechatPreviewImage": "Vista previa de la imagen",
  "share.downloadImage": "Descargar imagen",
  "share.downloadStarted": "Descarga iniciada.",
  "share.lang": "ES",
  lang: "ES",
};

const pt: Dict = {
  "nav.home": "Início",
  "nav.calculator": "Calculadora",
  "nav.program": "Programa CBT-I",
  "nav.assessment": "Avaliação",
  "nav.diary": "Diário",
  "nav.relax": "Relaxamento",
  "nav.learn": "Aprender",
  "nav.dashboard": "Painel",
  "nav.tools": "Ferramentas",
  "nav.tools.section": "Calculadoras de sono",
  "nav.guides": "Guias TCC-I",
  "nav.lessons": "Lições",
  "nav.blog": "Blog",
  "nav.faq": "Perguntas frequentes",
  "nav.pricing": "Planos",
  "nav.language": "Idioma",
  "nav.language.pt": "Português",
  "nav.language.en": "English",
  "nav.language.es": "Español",

  "cta.start": "Começar meu plano de sono",
  "cta.calc": "Experimentar a calculadora",
  "cta.continue": "Continuar meu plano",
  "cta.subscribe": "Assinar Premium",
  "cta.tryFree": "Testar 7 dias grátis",
  "cta.readMore": "Ler mais",
  "cta.backHome": "Voltar ao início",

  "hero.title": "Durma melhor, começando esta noite",
  "hero.sub":
    "O somna é seu companheiro de sono baseado em TCC-I. Sem remédios, sem pressão: pequenas mudanças diárias para recuperar seu ritmo natural de descanso.",
  "hero.badge": "TCC-I · Baseado em evidência clínica",

  "features.title": "Um caminho calmo de volta a noites reparadoras",
  "features.1.t": "Programa TCC-I",
  "features.1.d":
    "Seis semanas de hábitos suaves, passo a passo, para reconstruir seu ciclo natural de sono e acabar com a insônia.",
  "features.2.t": "Calculadora de ciclos",
  "features.2.d":
    "Descubra o horário ideal para dormir e acordar, baseado nos seus ciclos de sono de 90 minutos.",
  "features.3.t": "Relaxamento noturno",
  "features.3.d":
    "Técnicas de respiração, meditação e rituais calmantes para liberar o estresse antes de dormir.",

  "trust.title": "Com respaldo científico. Feito para a calma.",
  "trust.sub":
    "A terapia TCC-I é o tratamento sem medicamentos de primeira linha recomendado pelos especialistas em sono. Pequenas mudanças consistentes, ao longo de semanas, devolvem seu ritmo natural.",
  "trust.1.t": "Com evidência clínica",
  "trust.1.d":
    "Décadas de pesquisa científica sustentam a TCC-I como primeira opção para insônia crônica.",
  "trust.2.t": "Sem medicamentos",
  "trust.2.d":
    "Sem remédios, sem efeitos colaterais. Apenas hábitos que reeducam seu cérebro para dormir.",
  "trust.3.t": "Recuperação por hábitos",
  "trust.3.d": "Pequenos ajustes sustentáveis na sua rotina restauram o sono saudável com o tempo.",

  "emo.title": "Se suas noites parecem assim…",
  "emo.1": "Você fica acordado na cama muito tempo depois de apagar a luz",
  "emo.2": "A cabeça não para de pensar quando você tenta dormir",
  "emo.3": "Você está exausto, mas o sono não vem",
  "emo.4": "A ansiedade aparece conforme a hora de deitar se aproxima",
  "emo.note":
    "Você não está sozinho. O sono pode ser reaprendido: devagar, sem pressa e sem cobrança.",

  "calc.title": "Calculadora de ciclos de sono",
  "calc.sub":
    "O sono acontece em ciclos de cerca de 90 minutos. Acordar no fim de um ciclo ajuda você a se sentir descansado.",
  "calc.wake": "Quero acordar às",
  "calc.bed": "Quero dormir às",
  "calc.mode.wake": "Planejar pela hora de acordar",
  "calc.mode.bed": "Planejar pela hora de dormir",
  "calc.results.bed": "Tente deitar em um destes horários",
  "calc.results.wake": "Tente acordar em um destes horários",
  "calc.cycles": "ciclos",
  "calc.fall": "Considera cerca de 15 min para adormecer.",
  "calc.tier.4.label": "Sono mínimo",
  "calc.tier.5.label": "Ideal para a maioria dos adultos",
  "calc.tier.6.label": "Recuperação extra",
  "calc.tier.7.label": "Útil após privação de sono",
  "calc.badge.recommended": "⭐ Recomendado",
  "calc.badge.recovery": "Recuperação",
  "calc.explain.title": "Por que 5 e 6 ciclos são recomendados?",
  "calc.explain.body":
    "A maioria dos adultos funciona melhor com 7,5 a 9 horas de sono. Dormir em ciclos completos de 90 minutos pode ajudar a reduzir a letargia matinal e melhorar a energia durante o dia.",

  "program.title": "Programa TCC-I em 6 semanas",
  "program.sub":
    "Uma jornada suave de 6 semanas e 18 lições para reconstruir seu sono, um passo de cada vez.",
  "program.hubSub":
    "18 lições curtas e práticas, distribuídas em 6 semanas, para retreinar seu cérebro a dormir bem — sem remédios.",

  "assess.title": "Avaliação do sono",
  "assess.sub":
    "Uma breve e acolhedora revisão para entender seu sono e montar seu plano personalizado.",
  "assess.start": "Começar avaliação",
  "assess.support":
    "Muita gente vive isso. O sono pode melhorar aos poucos, com estrutura e constância.",
  "assess.next": "Próxima",
  "assess.back": "Voltar",
  "assess.done": "Ver meu perfil de sono",
  "assess.q1": "Quanto tempo você costuma levar para adormecer?",
  "assess.q2": "Com que frequência você acorda durante a noite?",
  "assess.q3": "Aproximadamente, quantas horas você dorme?",
  "assess.q4": "Como descreveria sua ansiedade na hora de deitar?",
  "assess.q5": "Quão descansado você se sente ao acordar?",
  "assess.result": "Seu perfil de sono",
  "assess.result.sub":
    "Um ponto de partida, não um diagnóstico. Seu plano TCC-I é montado em torno disso.",
  "assess.cta.program": "Começar programa",
  "assess.cta.dashboard": "Ir para o painel",
  "assess.level.label": "Perfil de sono",
  "assess.level1.name": "Dormidor saudável",
  "assess.level1.desc": "Seus hábitos de sono parecem saudáveis.",
  "assess.level2.name": "Dificuldades leves de sono",
  "assess.level2.desc": "Existem alguns desafios leves relacionados ao sono.",
  "assess.level3.name": "Padrão moderado de insônia",
  "assess.level3.desc": "As técnicas de TCC-I podem ajudar a melhorar seu sono.",
  "assess.level4.name": "Padrão significativo de insônia",
  "assess.level4.desc": "Recomendamos um programa estruturado de TCC-I.",
  "assess.health.label": "Saúde do sono",
  "assess.health.note": "Quanto mais alto, mais saudáveis são seus hábitos.",
  "assess.stat.efficiency": "Eficiência do sono",
  "assess.stat.efficiency.value": "No caminho certo",
  "assess.stat.program": "Programa recomendado",
  "assess.stat.program.value": "Jornada TCC-I",
  "assess.stat.duration": "Duração do programa",
  "assess.stat.duration.value": "6 semanas",
  "assess.stat.efficiency.value.l1": "Excelente",
  "assess.stat.efficiency.value.l2": "Bom",
  "assess.stat.efficiency.value.l3": "Razoável",
  "assess.stat.efficiency.value.l4": "Precisa de atenção",
  "assess.stat.program.value.l1": "Rotina de relaxamento",
  "assess.stat.program.value.l2": "Hábitos leves de TCC-I",
  "assess.stat.program.value.l3": "Programa central de TCC-I",
  "assess.stat.program.value.l4": "Programa completo de TCC-I",
  "assess.stat.duration.value.l1": "1–2 semanas",
  "assess.stat.duration.value.l2": "2–4 semanas",
  "assess.stat.duration.value.l3": "4–6 semanas",
  "assess.stat.duration.value.l4": "6 semanas",

  "diary.title": "Diário do sono",
  "diary.sub": "Um registro suave, sem julgamento, ajuda a revelar o que já está funcionando.",
  "diary.save": "Salvar registro",
  "diary.bedtime": "Hora de deitar",
  "diary.fall": "Tempo para adormecer (min)",
  "diary.wakes": "Despertares noturnos",
  "diary.wake": "Hora de acordar",
  "diary.quality": "Qualidade do sono",
  "diary.mood": "Humor ao acordar",
  "diary.cta.dashboard": "Ver painel",
  "diary.saved": "Registro salvo 🌙",

  "relax.title": "Relaxamento e descanso",
  "relax.sub":
    "Um espaço tranquilo para liberar o dia: respiração 4-7-8, meditação e rituais calmantes antes de dormir.",
  "relax.start": "Começar relaxamento",
  "relax.stop": "Parar",
  "relax.breathe": "Respiração 4-7-8",
  "relax.breathe.sub": "Inspire 4s · Segure 7s · Expire 8s",
  "relax.inhale": "Inspire",
  "relax.hold": "Segure",
  "relax.exhale": "Expire",

  "dash.title": "Seu painel do sono",
  "dash.sub":
    "Seu plano de sono para esta noite, sua eficiência e sua sequência de constância, em um relance.",
  "dash.bed": "Hora sugerida para dormir",
  "dash.wake": "Hora de acordar",
  "dash.eff": "Eficiência do sono",
  "dash.streak": "Sequência de constância",
  "dash.actions": "Ações rápidas",
  "dash.addEntry": "Abrir diário do sono",
  "dash.actions.title": "Ações rápidas",
  "dash.actions.log": "Diário",
  "dash.actions.relax": "Relaxar",
  "dash.actions.program": "Programa",
  "dash.actions.reminder": "Lembretes",
  "dash.chart.empty": "Ainda não há registros.",
  "dash.empty.body": "Pequenas mudanças geram progresso significativo.",
  "dash.empty.cta": "Registrar primeira noite",
  "dash.startWindDown": "Iniciar relaxamento",

  "learn.title": "Aprender",
  "learn.sub": "Leituras curtas para entender o sono e a você mesmo.",

  "common.save": "Salvar",
  "common.cancel": "Cancelar",
  "common.close": "Fechar",
  "common.continue": "Continuar",
  "common.back": "Voltar",
  "common.next": "Próximo",
  "common.confirm": "Confirmar",
  "common.delete": "Excluir",
  "common.edit": "Editar",
  "common.loading": "Carregando…",
  "common.retry": "Tentar novamente",
  "common.optional": "(opcional)",
  "common.required": "obrigatório",
  "common.yes": "Sim",
  "common.no": "Não",

  "error.generic.title": "Algo deu errado",
  "error.generic.body":
    "Ocorreu um erro da nossa parte. Você pode recarregar a página ou voltar ao início.",
  "error.retry": "Tentar novamente",
  "error.goHome": "Ir ao início",
  "error.404.title": "Página não encontrada",
  "error.404.body": "A página que você procura não existe ou foi movida.",

  "toast.saved": "Salvo com sucesso",
  "toast.deleted": "Excluído",
  "toast.copied": "Link copiado para a área de transferência",
  "toast.share.ok": "Pronto para compartilhar",
  "toast.share.fail": "Não foi possível compartilhar. Tente novamente.",

  "footer.tag": "Um companheiro de sono suave, guiado pela ciência.",
  "footer.rights": "© 2026 somna. Todos os direitos reservados.",
  "footer.disc":
    "Isso não é conselho médico. Se seus problemas de sono persistirem, consulte um profissional de saúde.",
  "footer.legal.privacy": "Privacidade",
  "footer.legal.terms": "Termos",
  "footer.legal.cookies": "Cookies",

  "home.return.title": "Continue seu caminho para dormir melhor",
  "home.return.cta": "Ir ao meu painel",

  "share.title": "Compartilhe seu progresso",
  "share.download": "Baixar imagem",
  "share.share": "Compartilhar",
  "share.cancel": "Cancelar",
  "share.reddit": "Reddit",
  "share.x": "X",
  "share.pinterest": "Pinterest",
  "share.facebook": "Facebook",
  "share.linkedin": "LinkedIn",
  "share.whatsapp": "WhatsApp",
  "share.pinterestFallback":
    "Primeiro gere a imagem para compartilhar e depois publique no Pinterest.",
  "share.copyLink": "Copiar link",
  "share.copied": "Link copiado",
  "share.copyFailed": "Não foi possível copiar o link",
  "share.failed": "Erro ao compartilhar. Tente novamente.",
  "share.uploadError": "Erro ao enviar a imagem. Verifique sua conexão.",
  "share.previewTitle": "Pré-visualização",
  "share.hidePreview": "Ocultar pré-visualização",
  "share.showPreview": "Mostrar pré-visualização",
  "share.errorTip": "Você ainda pode baixar a imagem e compartilhá-la manualmente.",
  "share.pngFormatInvalid": "Apenas imagens PNG são suportadas.",
  "share.fileSizeOverLimit": "A imagem deve ter menos de 5 MB.",
  "share.shareLesson": "Compartilhar esta lição",
  "share.shareProfile": "Compartilhar seu perfil de sono",
  "share.downloadProfile": "Baixar perfil",
  "share.shareProfileBtn": "Compartilhar perfil",
  "share.downloadResult": "Baixar resultado",
  "share.shareResult": "Compartilhar resultado",
  "share.sleepEfficiency": "Eficiência do sono",
  "share.improving": "Melhorando",
  "share.dayStreak": "Dias seguidos",
  "share.cbtiTraining": "Treinamento de sono TCC-I",
  "share.sleepProfile": "Perfil de sono",
  "share.sleepType": "Tipo de sono",
  "share.wechat": "WeChat",
  "share.weibo": "Weibo",
  "share.qq": "QQ",
  "share.wechatTitle": "Compartilhar no WeChat",
  "share.wechatScanHint": "Leia este QR Code com o WeChat",
  "share.wechatLongPress": "Mantenha a imagem pressionada para compartilhar.",
  "share.wechatPreviewImage": "Pré-visualizar imagem",
  "share.downloadImage": "Baixar imagem",
  "share.downloadStarted": "Download iniciado.",
  "share.lang": "PT",

  "lang.switch.title": "Idioma",
  "lang.switch.aria": "Mudar de idioma",
  "lang.switch.remember": "Lembrar meu idioma neste dispositivo",
  "lang.redirect.notice":
    "Levamos você à versão em português. Você pode mudar de idioma quando quiser, lá em cima.",

  lang: "PT",
};

const dicts: Record<Lang, Dict> = { en, zh, es, pt, pl };

const Ctx = createContext<{ lang: Lang; setLang: (l: Lang) => void; t: (k: string) => string }>({
  lang: "en",
  setLang: () => {},
  t: (k) => k,
});

export function I18nProvider({
  children,
  initialLang = "en",
}: {
  children: ReactNode;
  initialLang?: Lang;
}) {
  // El idioma lo decide el componente raíz según la ruta actual (SSR + cliente),
  // evitando mismatch de hidratación. initialLang viene de
  // useRouter().state.location.pathname en __root.tsx.
  const [lang, setLangState] = useState<Lang>(initialLang);

  // Si la ruta cambia en cliente (SPA), el prop initialLang cambia y
  // sincronizamos el estado.
  useEffect(() => {
    setLangState(initialLang);
    if (typeof window !== "undefined") window.localStorage.setItem("somna-language", initialLang);
  }, [initialLang]);

  const setLang = (l: Lang) => {
    setLangState(l);
    setUserLangPreference(l);
  };
  const t = (k: string) => {
    const localized = dicts[lang][k];
    if (localized !== undefined) return localized;
    return dicts.en[k] ?? k;
  };
  return <Ctx.Provider value={{ lang, setLang, t }}>{children}</Ctx.Provider>;
}

export const useI18n = () => useContext(Ctx);
