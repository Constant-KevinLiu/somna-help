...
export const Route = createFileRoute("/pl/program/week-3/sleep-restriction-mistakes")({
  loader: async () => {
    const lesson = await loadLesson("week-3", "sleep-restriction-mistakes", "pl");
  ...
  head: ({ loaderData }) => {
    ...
    return plLessonHeadFromLocales(lesson, "week3.lesson9");
  },
  notFoundComponent: () => (
  ...
  component: PlLesson9Route,
});

function PlLesson9Route() {
  const { lesson } = Route.useLoaderData();
  if (typeof document !== "undefined") {
    const title = plLessonTitleFromLocales("week3.lesson9");
