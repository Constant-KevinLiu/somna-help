...
export const Route = createFileRoute("/pl/program/week-3/sleep-restriction-therapy")({
  loader: async () => {
    const lesson = await loadLesson("week-3", "sleep-restriction-therapy", "pl");
  ...
  head: ({ loaderData }) => {
    ...
    return plLessonHeadFromLocales(lesson, "week3.lesson8");
  },
  notFoundComponent: () => (
  ...
  component: PlLesson8Route,
});

function PlLesson8Route() {
  const { lesson } = Route.useLoaderData();
  if (typeof document !== "undefined") {
    const title = plLessonTitleFromLocales("week3.lesson8");
