...
export const Route = createFileRoute("/pl/program/week-4/racing-thoughts-at-night")({
  loader: async () => {
    const lesson = await loadLesson("week-4", "racing-thoughts-at-night", "pl");
  ...
  head: ({ loaderData }) => {
    ...
    return plLessonHeadFromLocales(lesson, "week4.lesson10");
  },
  notFoundComponent: () => (
  ...
  component: PlLesson10Route,
});

function PlLesson10Route() {
  const { lesson } = Route.useLoaderData();
  if (typeof document !== "undefined") {
    const title = plLessonTitleFromLocales("week4.lesson10");
