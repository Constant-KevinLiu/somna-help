...
export const Route = createFileRoute("/pl/program/week-4/breathing-exercises-for-sleep")({
  loader: async () => {
    const lesson = await loadLesson("week-4", "breathing-exercises-for-sleep", "pl");
  ...
  head: ({ loaderData }) => {
    ...
    return plLessonHeadFromLocales(lesson, "week4.lesson12");
  },
  notFoundComponent: () => (
  ...
  component: PlLesson12Route,
});

function PlLesson12Route() {
  const { lesson } = Route.useLoaderData();
  if (typeof document !== "undefined") {
    const title = plLessonTitleFromLocales("week4.lesson12");
