...
export const Route = createFileRoute("/pl/program/week-4/relaxation-techniques")({
  loader: async () => {
    const lesson = await loadLesson("week-4", "relaxation-techniques", "pl");
  ...
  head: ({ loaderData }) => {
    ...
    return plLessonHeadFromLocales(lesson, "week4.lesson11");
  },
  notFoundComponent: () => (
  ...
  component: PlLesson11Route,
});

function PlLesson11Route() {
  const { lesson } = Route.useLoaderData();
  if (typeof document !== "undefined") {
    const title = plLessonTitleFromLocales("week4.lesson11");
