...
export const Route = createFileRoute("/pl/program/week-6/preventing-relapse")({
  loader: async () => {
    const lesson = await loadLesson("week-6", "preventing-relapse", "pl");
  ...
  head: ({ loaderData }) => {
    ...
    return plLessonHeadFromLocales(lesson, "week6.lesson16");
  },
  notFoundComponent: () => (
  ...
  component: PlLesson16Route,
});

function PlLesson16Route() {
  const { lesson } = Route.useLoaderData();
  if (typeof document !== "undefined") {
    const title = plLessonTitleFromLocales("week6.lesson16");
