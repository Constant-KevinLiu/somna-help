...
export const Route = createFileRoute("/pl/program/week-5/realistic-sleep-expectations")({
  loader: async () => {
    const lesson = await loadLesson("week-5", "realistic-sleep-expectations", "pl");
  ...
  head: ({ loaderData }) => {
    ...
    return plLessonHeadFromLocales(lesson, "week5.lesson15");
  },
  notFoundComponent: () => (
  ...
  component: PlLesson15Route,
});

function PlLesson15Route() {
  const { lesson } = Route.useLoaderData();
  if (typeof document !== "undefined") {
    const title = plLessonTitleFromLocales("week5.lesson15");
