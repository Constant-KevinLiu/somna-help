/**
 * Página de Relajación en español (/es/relajacion).
 *
...
export const Route = createFileRoute("/es/relajacion")({
  head: () => ({
    meta: [
    ...
    links: hreflangLinks("/es/relajacion"),
  }),
  component: EsRelaxPage,
});

function EsRelaxPage() {
  const t = loadEsDict();
    ...
    <section className="relative overflow-hidden px-5 pt-16 pb-20 md:pt-24">
      ...
      <div className="mx-auto max-w-2xl text-center animate-fade-up">
        ...
        <div className="mt-8">
          ...
          >
            {t["relax.start"]} <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
        <p className="mt-6 text-sm text-muted-foreground">
          {t["relax.breathe"]} · {t["relax.breathe.sub"]}
        </p>
      </div>
    </section>
  );
